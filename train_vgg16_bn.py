...
from vgg_16_bn import model_VGG16_BN
...
wandb.init(project="master_thesis", config=defaults, name="vgg16_bn_2k")
config = wandb.config
...
model = model_VGG16_BN(32, 2, 128, 32, len(mlb.classes_))
...
callbacks = [
    ...
    ModelCheckpoint('model_vgg16_bn_final_2k.h5', verbose=1, save_best_only=True,save_weights_only=False)
]
...
model.save("model_vgg16_bn_final_2k")
# save the multi-label binarizer to disk
print("[INFO] serializing label binarizer...")
f = open("labelbinvgg16_bn_final_2k", "wb")
f.write(pickle.dumps(mlb))
f.close()

#save model
wandb.save('model_vgg16_bn_final.h5')
...
df.to_csv('Classification_Report_vgg16_final_5k.csv', index= True)
...
#save txt file
with open("my_list_bn_final_newdata_minmax.txt", "w") as output:
...
def plot_ml_confusion_matrix(testY, y_pred, label_to_class, save_plot=False):
    ...
    for i, conf_matrix in enumerate(multilabel_confusion_matrix(testY, y_pred)):
        ...
        plt.tight_layout()
    if save_plot:
        plt.savefig('confusion_matrices_vgg_bn.png')
